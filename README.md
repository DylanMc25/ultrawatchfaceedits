...
The current layout is standalone `:app` version **0.1.9 / 10** and Samsung forecast preview **0.2.0-preview.6 / 13**. The original standalone face remains available on Wear OS 5+ as `com.example.ultrainfoboard`; the bundled preview requires Wear OS 6. Version 13 keeps the familiar staggered layout, evens the circle gaps, makes the date bold, and limits the forecast picker to image panels. This revision passes native checks on small/large API 36 and standalone API 34/35; physical readability and upgrade retention still need review. Final branding, package identity and release signing remain release prerequisites.
