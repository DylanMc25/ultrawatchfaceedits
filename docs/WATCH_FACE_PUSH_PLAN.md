...
The host and bundled face use API 36 as their minimum, matching the accepted Wear OS 6 requirement. Version 14 retains editable complication IDs 1–6, retires editable slot 7 and uses fixed slot 8 with `SamsungForecastService` as its default. The provider fills the 262×94 bottom rectangle according to the app's four-choice chart menu. Each chart opens Samsung Weather; None supplies a transparent image without a tap action. The provider and existing permission remain in the same host package. [Current menu and installation instructions](SAMSUNG_FORECAST_TESTING.md)
