...
- Verify all seven Bottom panel choices and their persistence on supported Galaxy Watch firmware. Native weather must show real current/hourly data, missing/stale states and correct units; weather panels must open Samsung Weather from the whole rectangle. Steps and Heart rate must behave correctly with granted/denied access and launch the intended health destination. None must be noninteractive.
- Check native weather freshness/location setup separately from Samsung's public complication provider: this face draws supported WFF weather fields rather than requesting a rich forecast from that provider. Historical Samsung Health charts remain outside this resource-only build; see [compatibility findings](SAMSUNG_WEATHER.md).
- Confirm forecast hour labels follow the device time zone, daylight-saving changes and 12/24-hour preference. Temperature units come from the native weather source. Verify consistently signed upgrades preserve slots 1–6 and replace the former slot 7 assignment with the Bottom panel default.
