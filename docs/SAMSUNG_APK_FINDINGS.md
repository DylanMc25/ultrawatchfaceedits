...
The host and embedded face remain separate installed package identities. [Watch Face Push](https://developer.android.com/training/wearables/watch-face-push) requires the face name under the host's `.watchfacepush.` prefix, separate face signing, strict APK contents and an official validation token. This would require a migration from the existing development face identity. The user subsequently accepted Wear OS 6 as the minimum requirement. The original API 34/35 baseline is therefore no longer required for the proposed bundled architecture. The repository build has not yet been migrated.
...
The user requires the weather rectangle to open Samsung Weather when tapped. Forecast source and tap destination are independent decisions. Google's [SmallImageComplicationData.Builder](https://developer.android.com/reference/androidx/wear/watchface/complications/data/SmallImageComplicationData.Builder#setTapAction(android.app.PendingIntent)) supports a provider-supplied PendingIntent. Android's [package launch API](https://developer.android.com/reference/android/content/pm/PackageManager#getLaunchIntentForPackage(java.lang.String)) obtains the normal entry activity for another app.

The supplied WeatherWatch manifest declares an exported MAIN/LAUNCHER alias `com.samsung.android.watch.weather.ui.WeatherActivity` targeting `com.samsung.android.watch.weather.WeatherActivity`. The user already verified a package launch from our old native panel opens Samsung Weather. This is separate from—and does not require—querying Samsung's protected hourly provider.
