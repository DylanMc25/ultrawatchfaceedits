...
The current source is **0.2.0-preview.5 / version 12**. It keeps the familiar layout, gives the three 96-unit circles equal spacing, aligns day/date over the upper circle, and makes the active date bold. The 262 × 94 forecast panel is unchanged. Its standard picker now accepts only small-image panels or Empty, excluding generic long-text choices. Other slots remain unchanged. Validation of this revision is recorded in [the validation log](VALIDATION.md); older runs are not evidence for this update.
...
4. Select **Ultra Forecast** in the watch-face picker. This distinct name separates it from the older **Ultra Info Board** face. If it is missing, return to the app and tap **Install or update watch face**. The bottom rectangle defaults to **Weather** from **Ultra Info Board Weather** in version 12; older previews call this source **Samsung hourly forecast**.
