...
This preview reads Samsung Weather's saved forecast through its existing user-granted weather permission. It does not contact a replacement weather service, copy Samsung artwork, or modify Samsung's apps. On 2026-09-21 (user's local date), the user installed the CI preview on the API 36 Galaxy Watch and reported that its forecast appeared correct. They confirmed it appears in the face's rectangle and tapping opens Samsung Weather, then reported it still was not selectable like a normal complication. Data/display/tap checks succeeded on that earlier build; Samsung customization and the version 13 layout remain unverified.
