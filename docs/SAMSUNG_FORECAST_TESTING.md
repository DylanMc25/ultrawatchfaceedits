...
The current source is **0.2.0-preview.5 / version 12**. It keeps the familiar layout, gives the three 96-unit circles equal spacing, centers the two date lines together with a tighter line gap, and makes the active date bold. The 262 × 94 forecast panel is unchanged. Its standard picker now accepts only small-image panels or Empty, excluding generic long-text choices. The left edge has 18 ticks and the right arc spans 40 degrees. The bottom shortcut accepts monochrome icons or Empty; its former text/image-only assignments may need replacing. Circle and edge formats stay unchanged. Validation of this revision is recorded in [the validation log](VALIDATION.md); older runs are not evidence for this update.
