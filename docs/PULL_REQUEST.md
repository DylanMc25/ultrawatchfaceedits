...
- A native-picker test replaces the rectangle with Alarm, checks persistence and taps, verifies setup does not overwrite that choice, and restores the forecast source. Version 9 also distinguishes the face as **Ultra Info Board Forecast** and adds a source icon and exact selection guidance. These identification improvements are not a proven fix for the physical report.
