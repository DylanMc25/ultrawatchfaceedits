...
All six slots independently open their native provider chooser on both emulators. Steps, sunrise/sunset and battery taps dispatch to the correct slots, and battery opens system Battery settings. The native editor also renders the populated weather layout using its own sample data. The assigned Alarm edge label is also fully visible on both sizes. Both weather tap areas request the correct Samsung Weather package without triggering a complication. The stock emulators lack Samsung Weather, so its actual launch remains a physical-device check, alongside real health-provider launches, live weather and provider persistence across updates/reboots. [Passing build, validation and emulator run](https://github.com/DylanMc25/ultrawatchfaceedits/actions/runs/35473692442).
