...
A Wear OS 6 host bundles a separately signed resource-only face using Watch Face Push. Version 12 keeps the two-line date and staggered circles, makes the active date bold, and makes all three circles 96 units with equal gaps. The forecast remains 262 × 94. Its provider is named **Weather** under **Ultra Info Board Weather**. The rectangle accepts image panels (`SMALL_IMAGE`) or Empty; the six other slots retain their formats and all seven IDs stay stable. Wear OS determines eligible apps. Older long-text rectangle assignments need replacing.
...
- Version 12 local APK, test APK, unsigned host AAB and lint checks pass; 29 JVM and 18 Python tests pass.
- Official Push validation passes all ten checks on the signed embedded face. Standalone v9 APK/AAB, WFF schema and memory checks pass.
